...
class BornAgainTree:
    def __init__(self):
        self._forestMaxDepth = 0
        self._forestNumEstimators = 0
    ...
    def writeForestOnFile(self, trees):
        ...
        # node
            ...
            is_split_node = children_left[node_id] \
        ...
        # Credo che vada tolto nel nostro codice, ma non ne sono sicuro
        relativePath = 'src/resources/forests/Loan/' \
            + self._filename[0:len(self._filename) - 3] + 'RF' + str(self._numFold) \
            + '.txt'
        absolutePath = os.path.join(scriptDir, relativePath)
        f = open(absolutePath, 'w')
        f.write('DATASET_NAME: ' + self._filename + '\n')
        f.write('ENSEMBLE: RF\n')
        f.write('NB_TREES: ' + str(self._forestNumEstimators) + '\n')
        f.write('NB_FEATURES: ' + str(self._numFeatures) + '\n')
        f.write('NB_CLASSES: 2\n')
        f.write('MAX_TREE_DEPTH: ' + str(self._forestMaxDepth) + '\n')
        ...
        for j in range(0, len(trees)):
            ...
            for i in range(n_nodes):
                ...
                if is_leaves[i]:
                    f.write('LN ')
                    f.write(str(children_left[i]) + ' '
                            + str(children_right[i]) + ' '
                            + str(feature[i]) + ' ' + str(threshold[i])
                            + ' ' + str(node_depth[i]) + ' ')
    ...
    def prepareData(
        ...
        # Credo che vada tolto nel nostro codice, ma non ne sono sicuro
        if isTrain:
            relativePath = 'src/resources/datasets/Loan/' \
                + self._filename[0:len(self._filename) - 3] + 'train' \
                + str(numFold) + '.csv'
        else:
            relativePath = 'src/resources/datasets/Loan/' \
                + self._filename[0:len(self._filename) - 3] + 'test' + str(numFold) \
                + '.csv'
        absolutePath = os.path.abspath(os.path.join(scriptDir, relativePath))
        data = pd.concat([data_x, data_y], axis=1)
        data.to_csv(absolutePath, index=False)

    def createRandomForestByCV(self,
        train_x,
        train_y,
        forestMaxDepth,
        forestNumEstimators,
        ...
        self._forestMaxFeatures = forestMaxFeatures
        self._numFeatures = train_x.columns.size
        self._numFold = numFold
        myclassifier = RandomForestClassifier(random_state=42,
                criterion=forestCriterion,
                n_estimators=forestNumEstimators,
                max_features=forestMaxFeatures,
                max_depth=forestMaxDepth)

        params = {}

        gscv_rf = GridSearchCV(myclassifier, param_grid=params,
                               scoring=metric, cv=numFold, refit=True)
        ...
        self.writeForestOnFile(trees)

    def getBornAgainTree(self, test_x, test_y):
        self.prepareData(test_x, test_y, False, self._numFold)

        current_dataset = 'Loan'
        current_fold = self._numFold
        n_trees = self._forestNumEstimators
        current_obj = 4
        using_cplex = False

        (df_train, df_test, ds_infos) = ds.load(current_dataset,
                current_fold)
        ...
        (random_forest, random_forest_file) = rf.load(
            X_train,
            y_train,
            current_dataset,
            current_fold,
        ...
        if 0 \
            == os.system('make --directory=./src/born_again_dp {} > buildlog.txt'.format(('withCPLEX=1'
                          if using_cplex else ''))):
        ...
        born_again_file = '{}.BA{}'.format(current_dataset,
                current_fold)
        ret = subprocess.run([
            './src/born_again_dp/bornAgain',
            random_forest_file,
            born_again_file,
            '-trees',
            str(n_trees),
        ...
        born_again = tree_io.classifier_from_file(born_again_file
                + '.tree', X_train, y_train, pruning=False)
